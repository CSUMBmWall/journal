...
When inserting a YouTube link, the Node.js server makes a call to  the Google YouTube API that grabs the title and a thumbnail image.  It inserts that as a hyperlink.  I tried using an iframe, but the Angular pipe I use to convert Markdown reloads the iframe image every time I type anything in the edit pane, and it gets on my nerves.  
...
There are Edit Post, Delete, and bookmark buttons on the top of the post.  
